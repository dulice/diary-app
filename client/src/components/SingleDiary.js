...
const SingleDiary = () => {
  const navigate = useNavigate();
    ...
    const handleEdit = async () => {
      setLoading(true);
      try {
        const { data } = await axios.put(`/api/diaries/${id}`, {
          title,
          description,
        });
        setEdit(false);
    ...
    <div className='max-w-6xl mx-auto px-2'>
        ...
        {   loading ? <div>Loading...</div>
            ...
            <motion.div
                ...
                initial="hidden"
                animate="visible"
                className="max-w-6xl px-2 mx-auto my-5">
                ...
                {edit ? (
                    ...
                    <input 
                      ...
                      type="text" 
                      placeholder='Title' 
                      className='mb-3 font-medium text-xl focus:outline-none block'
                      required
                      autoFocus
                    />
                ...
                {edit ?
                  <>
                    ...
                    {loading ? <button className='bg-blue-500 mb-5 rounded-sm px-2 py-1 text-white float-right'>Updating</button>
                    :
                    <motion.button 
                      variants={addButton}
                      initial="hidden"
                      ...
                      whileHover="hover"
                      onClick={handleEdit}
                      type='submit' className="bg-blue-500 mb-5 rounded-sm px-2 py-1 text-white float-right">Update</motion.button> 
                    }
                  </>
